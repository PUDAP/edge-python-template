...
Point this **edge host** at the site NTP server (Chrony on the NATS host). The
edge process does not talk NTP; it uses the host clock.

Do not run these scripts on the NATS/NTP machine. That host already runs the
`chrony` container, which sets its clock.

Pass the server address when you run the script. Use `HOST_IP` from the site
Chrony `.env` (Tailscale IP, LAN IP, or Tailscale MagicDNS). Example:
